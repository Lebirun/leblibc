...
LIBC_OBJS = $(filter obj/src/%,$(ALL_OBJS)) $(filter obj/compat/%,$(ALL_OBJS)) obj/src/leb_syscalls.o
...
CFLAGS_C99FSE = -std=c99 -ffreestanding -nostdinc -Wa,--noexecstack
...
TOOL_LIBS = lib/leblibc-gcc.specs
ALL_LIBS = $(CRT_LIBS) $(STATIC_LIBS) $(SHARED_LIBS) $(EMPTY_LIBS) $(TOOL_LIBS)
ALL_TOOLS = obj/leblibc-gcc

WRAPCC_GCC = gcc
WRAPCC_CLANG = clang

LDSO_PATHNAME = $(syslibdir)/ld-leblibc-$(ARCH)$(SUBARCH).so.1
...
obj/%.lo: $(srcdir)/%.c $(GENH) $(IMPH)
	$(CC_CMD)
obj/src/leb_syscalls.o: ../src/leb_syscalls.c $(GENH) $(IMPH)
	$(CC) $(CFLAGS_ALL) -c -o $@ $<
...
lib/leblibc-gcc.specs: $(srcdir)/tools/leblibc-gcc.specs.sh config.mak
	sh $< "$(includedir)" "$(libdir)" "$(LDSO_PATHNAME)" > $@

obj/leblibc-gcc: config.mak
	printf '#!/bin/sh\nexec "$${REALGCC:-$(WRAPCC_GCC)}" "$$@" -specs "%s/leblibc-gcc.specs"\n' "$(libdir)" > $@
	chmod +x $@
